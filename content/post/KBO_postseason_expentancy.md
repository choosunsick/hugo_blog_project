...
안녕하세요. 스터디 그룹 LOPES의 추선식입니다. 메이저리그 야구 통계학라는 책을 공부하는데, 이 책에서 나온 기대 승률(피타고리안 승률)이란 통계적 지표를 KBO 자료에 적용해보았습니다. 이 책에서는 기대 승률을 다음과 같이 설명합니다.
...
{{< tex display="\ 오클랜드(2001): \dfrac{884^2}{(884^2 + 645^2)} = 0.6525  (2001년 실제 승률은 0.6296)" >}}
...
{{< tex "\dfrac{득점^2} {(득점^2 + 실점^2)} = 기대 승률">}}

> 공식의 왼쪽 항에 있는 분수식을 논리적으로 이해할 필요는 없다. 경험적으로 득점과 실점을 이렇게 구성했을 때 실제 승률과 가장 비슷한 기대 승률이 나왔다는 경험적 발견이기 때문이다.

> \[메이저리그 야구 통계학, 김재민, 에이콘출판, pp.119 ~ 120\]

현재 프로야구는 올스타전이 끝나고 후반기에 돌입하여 가을 야구까지 약 2~3개월이 남은 상태입니다. 많은 사람이 어느 팀이 포스트시즌 진출해 가을야구를 할 것인지 궁금해할 것입니다. 기대 승률이라는 지표와 실제 승률과 전년도 포스트시즌 진출팀의 평균 승률과 같은 통계적 지표들 간의 비교를 통해 어느 팀이 포스트시즌에 진출할 가능성이 높은지를 살펴보겠습니다.

# 기대 승률 및 실제 승률 비교 코드

## 자료를 열어 포스트시즌 경기팀 뽑기

이제 제가 수집한 자료를 R로 읽어와 보겠습니다. 다음은 2017년 자료를 열어서 읽어오는 코드입니다.
...
읽어온 자료에서 정규시즌과 시범경기 올스타전을 제외한 경기만 뽑아줍니다. 이어서 그 자료에서 홈팀과 원정팀을 뽑아 고유한 리스트로 만들어줍니다. 결과를 보시면 "NC", "롯데", "두산","기아", "SK"의 2017년도 포스트시즌 진출 5개팀을 확인할 수 있습니다.
...
## 전년도 포스트시즌 진출팀 승률의 평균 값 구하기


이제 이 5개 팀의 정규시즌 승률의 평균을 함수를 통해 구할 수 있습니다. 구해진 평균 승률은 이후에 현재 상위권 팀의 기대 승률과 비교하는 데 사용됩니다. 이 과정에서 필요한 함수는 두 가지로 데이터 전처리 함수와 승률 계산하기 위한 함수입니다. 전처리함수는 데이터와 연도 팀이름의 3가지 인자를 받는 함수로 인자로 받은 팀과 해당 연도의 정규시즌 홈과 원정에서의 경기결과를 새롭게 만들어 주는 역할을 합니다. 이 함수에 대해 자세히 설명하면 다음과 같이 작동합니다. 먼저 데이터에서 해당 팀이 홈팀일 때 정규시즌 경기기록을 찾아 데이터를 뽑아냅니다.

이후 경기연도를 열을 만들어 입력된 년도의 데이터만 뽑아냅니다. 가장 중요한 경기결과 열을 만드는데 홈팀점수와 원정팀점수 간의 비교를 통해 홈팀점수가 더 높으면 승 낮으면 패 같으면 무로 인덱싱을 하여 경기결과 열을 구성해줍니다. 이런 과정을 인자로 받은 팀이 원정팀인 경우에도 똑같이 진행하여 경기결과 열을 만들어 줍니다. 그러면 이제 해당팀의 정규시즌 전체 경기결과 열이 만들어지기에 두 데이터 다시 합쳐서 반환하여 줍니다. 이렇게 반환된 데이터는 다음과 같습니다.
...
전처리 함수에서 반환된 데이터를 승률을 구하는 함수에 넣어서 해당 팀의 정규시즌 승, 패, 무, 승률에 대한 계산을 합니다. 승률 함수는 전처리 함수에서 만든 경기결과 열을 통해 해당 팀의 승리, 패배, 무승부(있는 경우에만)를 가지고 승률 공식에 대입하여 계산하는 역할을 합니다. 승률 공식은 현재 총 승리한 경기수/(전체 경기에서 무승부를 제외한 경기 수)로 계산합니다. 함수에 대해 자세히 설명하면 다음과 같습니다. 먼저 인자는 하나로 데이터만 받습니다. 그리고 해당 데이터가 2010년의 데이터인지 아닌지를 검사하는데 이는 2010년에는 승률을 구하는 공식이 다르기 때문입니다.

이제 데이터가 2010년이 아닌 경우면 위의 공식으로 게산을 하게됩니다. 그후에 해당 팀의 기록에 무승부가 있는지 없는지를 검사합니다. 이것은 승률 계산시 무승부를 제외하기에 필요한 검사입니다. 이런 검사 이후에 무승부가 있는 경우와 없는 경우를 나누어 해당 팀의 승률을 계산해줍니다. 이렇게 계산된 승률은 해당 팀의 승리와 패배, 무승부 기록과 함께 데이터 프레임(data.frame)으로 반환됩니다. 함수의 결과는 다음과 같습니다.
...
이제 같은 방식으로 앞서 구한 전년도 포스트시즌 진출한 \["NC","롯데","두산","기아","SK"\] 5개 팀 모두의 승률을 구해줍니다. 5개 팀의 승률을 구하면 그것의 평균을 구해줍니다. 과정과 결과는 다음과 같습니다.
...
이제 작년 포스트시즌 진출팀의 평균 승률을 구했으니 비교할 대상인 현재 시즌에 상위팀 "두산","한화","SK","LG","넥센"의 전반기 승률과 기대 승률을 구해볼 차례입니다. 먼저 2018년 전반기 데이터를 읽어와줍니다. 앞서와 마찬가지로 전처리 함수와 승률 함수를 통해 5팀의 승률 기록을 구해줍니다.
...
("두산","한화","SK","LG","넥센") 팀의 승률을 구하면 다음과 같습니다.
...
이제 다른 비교지표인 기대 승률을 구해보겠습니다. 기대 승률을 구하는 것 역시 함수를 통해 이루어집니다. 이 함수는 데이터와 팀이름을 인자로 받습니다. 인자로 받은 팀이름을 홈팀과 원정팀일 때로 구분하여 각각 득점과 실점의 합을 구하고 그것을 통해 기대 승률 공식에 대입하여 기대 승률과 총득점 총실점을 반환하는 함수입니다.
...
## 현재 상위5개팀의 2017년도 총득점, 총실점 구해보기

전년도인 2017년도에 현재 상위 5개팀의 총득점과 총실점을 구해본다. 구하는 방법은 작년의 데이터를 기대 승률 함수에 적용한다. 결과는 다음과 같다. 책에서와 같이 여기서 구한 총실점을 이번년도 총실점에도 그러할 것이라 가정한다.
...
이제 총실점을 전년도 것으로 놓고 전년도 포스트시즌 진출팀의 평균 승률인 57%를 맞추려면 시즌 종료까지 총 몇 득점을 더 해야 하는지 추정해보겠습니다. 추정 방법은 다음과 같이 기대 승률을 구하는 공식을 역추하는 함수를 통해서 득점을 추정하는 것입니다. 즉 전년도 총 실점과 전년도 포스트시즌 진출팀의 평균 승률을 인자로 받으면서 미지수인 총득점의 제곱 값을 sqrt()로 풀어주는 것을 통해 목표득점을 추정해 볼 수 있습니다. 최종적으로는 위에서 구한 현재까지의 총득점과 목표득점 간의 차를 통해 시즌 말까지의 요구득점을 알아낼 수 있습니다.
...
이렇게 추정한 정보와 더불어 현재 기대 승률과 실제 승률 그리고 전년도 포스트시즌 진출팀의 평균 승률 간의 비교를 통해 상위 5개 팀의 가을 야구 가능성을 점쳐 볼 수 있습니다. 현재 기대 승률과 평균 승률 간 비교하는 열은 전반기까지의 성적만으로도 포스트시즌에 가기에 충분한지 즉 기대 승률이 평균 승률과 같거나 높으면 전반기와 같은 득점과 실점 추세를 유지한다면 충분히 올라갈 수 있다는 의미입니다. 이어서 기대 승률과 실제 승률 간 비교 열은 전반기에 해당팀이 얼마나 전략적으로 잘해왔는지 혹은 운이 좋았는지를 말해줍니다.
...
두산과 SK만 현재 기대 승률이 작년 포스트시즌 진출팀의 평균 승률을 넘고 나머지 팀은 넘지 못하는 것을 알 수 있습니다. 두산의 경우 이러한 추세만 유지한다면 문제없이 가을야구를 하게 될 것입니다. SK 역시 기대 승률이 57%를 유지하지만 아슬아슬하게 유지하고 있기에 득점을 좀 더 늘려야 안정적으로 포스트시즌에 진출할 수 있을 거라 생각됩니다. 넥센과 LG의 경우에도 득점을 늘리고 실점을 줄여야 안정적으로 57%의 이상의 기대 승률을 달성할 것으로 생각됩니다. 물론 LG의 경우 필요로 하는 득점의 수가 그렇게 많지 않은 점을 감안하면 넥센보다는 수월하다고 생각됩니다.

그러나 특이하게도 한화의 경우 현재 2위지만 기대 승률은 49%로 굉장히 낮은점을 찾아 볼 수 있습니다. 이런 모습의 원인은 한화는 득점보다 실점이 많기 때문입니다. 물론 실제 승률은 높지만, 그것은 전반기에 운이 좋았다라고 말할 수 있습니다. 따라서 만약 이러한 추세가 계속될 경우 한화는 실점이 많아지기에 순위가 내려갈 가능성이 있습니다. 즉 한화는 현재 순위를 유지하기 위해서는 요구하는 득점이 가장 많은점을 보아 앞으로 득점력을 많이 올려야 하며 총실점이 총득점보다 많기에 실점 자체를 줄여야 한다고 생각됩니다. 그렇지 않으면 한화의 경우 앞으로 현재 순위를 유지하기 힘들 것으로 생각됩니다.

## 10개 팀 결과

밑의 코드는 10개의 전체팀 결과를 보여줍니다.
