...
fn main() {
    let now = Instant::now();
    println!("{:?}",simulation(100));
    let after = Instant::now();
    println!("100회 시뮬레이션 시간: {:?}", after.checked_duration_since(now));
    println!("{:?}",simulation(1000));
    println!("{:?}",simulation(10000));
}
...
```bash
3.1418064
100회 시뮬레이션 시간: Some(171.268668ms)
3.1416152
3.1415677
```

파이썬의 경우 평균 9.3초가 걸렸으며, 밀리초로 환산하면 9300밀리초가 됩니다. rust의 경우 약 171 밀리초가 걸렸습니다. 두 코드의 작동 시간 차이는 약 50배로 어마어마한 차이를 보입니다. 그러나 추정된 값의 차이는 큰 차이가 없는 것으로 보입니다. 두 코드의 파이 추정값은 3.141까지 맞춘것으로 동일하기 때문입니다. 참고로 찍는 점의 개수를 10만개 보다 더 많이 할 경우 파이값의 추정치가 더 정확해지게 됩니다.

그렇다면 시뮬레이션 회수를 늘리면 파이값 추정치는 어떻게 될까요? 러스트의 경우 1000번 시뮬레이션의 경우 3.1416의 값으로 3.1415값에 가까워졌습니다. 10000번 시뮬레이션 결과를 보면 파이 추정치 값이 실제 파이값의 3.1415까지 맞추는 것을 확인할 수 있습니다. 파이썬의 경우도 마찬가지로 1000번 시뮬레이션한 결과 3.1416의 값으로 3.1415 값에 더 가까워졌습니다. 참고로 파이썬의 경우 10000번 시뮬레이션 할 경우 시간이 상당히 오래 소요됩니다. 수행 결과 3.141648096로 추정값 자체는 3.1415로 점차 작아지는 모습을 확인할 수 있습니다. 이에따라 파이 값의 추정치는 시뮬레이션 회수와 점의 개수에 모두 영향 받는다는 점을 알 수 있습니다. 